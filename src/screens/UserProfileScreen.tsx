...
const UserProfileScreen = ({ navigation, route }: any) => {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#F8F8F8", paddingTop: sh(40), paddingBottom: sh(20) }}>
        {/* ── Header ── */}
        ...
        <ScrollView ref={scrollRef} showsVerticalScrollIndicator={false} style={{ flex: 1 }}>
          ...
          {!!user.attributes?.length && (
            <Section title="Physical attributes" style={{ minHeight: 107 }}>
          ...
          {!!user.interests?.length && (
            <Section title="Interests" style={{ minHeight: 152 }}>
              <Wrap style={{ marginTop: sh(2) }}>
...
const Footer = ({ onBackToTop }: { onBackToTop: () => void }) => (
  <View style={{ alignItems: "center", marginVertical: sh(30) }}>
    <TouchableOpacity onPress={() => showToast({ text1: "User Blocked", icon: X })}>
