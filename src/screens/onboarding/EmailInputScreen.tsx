...
import { signupStartEmailSchema, SignupStartResponse } from '@/features/auth/schema';
...
export default function EmailInputScreen({ navigation }: any) {
  ...
  const onValid = (data: { email: string }) => {
    signupStart(
      { email: data.email },
      {
        onSuccess: async (res: SignupStartResponse) => {
          await SecureStore.setItemAsync('signupSessionId', res.signupSessionId);
          showToast({ text1: 'Verification code sent' });
          // ✅ Pass `email` (not `phone`) so NumberVerifyScreen knows the channel
          navigation.navigate('NumberVerifyScreen', { email: data.email });
        },
        onError: (err: any) => {
          showToast({
      ...
      <View style={{ flex: 1, backgroundColor: '#ffffff' }}>
        ...
        >
          <View style={{ paddingHorizontal: sw(20), paddingTop: sh(72) }}>
            ...
            {/* Header */}
            <View style={{ marginTop: sh(32), marginBottom: sh(32), gap: sh(6) }}>
